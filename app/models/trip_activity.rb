class TripActivity < ApplicationRecord
  ...
  scope :find_trip_activities, -> (trip_id, date) { where(trip_id: trip_id).where("start_at >= ? AND start_at <= ?", DateTime.parse(date), DateTime.parse(date).end_of_day) }
